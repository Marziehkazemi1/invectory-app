        ...
        products.forEach((item) => {
            ...
            result += ` <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-x-3">
                    ...
                    <span class="block px-3 py-0.5 text-slate-400 border border-slate-400 text-sm rounded-2xl">${item.category}</span>
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-slate-500 
                    border-2 border-slate-300 text-slate-300">${item.quantity}</span>
                    <button class="delete-product border px-2 py-0.5 rounded-2xl border-red-400 text-red-400" data-product-id=${item.id}>delete</button>
                </div>
                </div>`
        });
